...
$(document).ready(function(){
    ...
    var oList = {
        init:function(){
            ...
            // this.getData();
            this.getExamine();//判断是否审核通过
        },
        getExamine:function(){
            //ajax 获取数据
            var data = 1
            if(data==1){
                $('.index-state-info').css({'display':'none'})
                $('.index-state-info-no').css({'display':'block'})
                $('.sup-index-no').css({'display':'block'});
                var opacityNum = 1;
                var timer = setInterval(function(){
                    opacityNum-=0.05
                    if(opacityNum<=0){
                        $('.sup-index-no').css({'display':'none'});
                        clearInterval(timer);
                    }else{
                        $('.sup-index-no').css({'opacity':opacityNum});
                    }
                },100)
            }else{
                $('.index-state-info').css({'display':'block'})
                $('.index-state-info-no').css({'display':'none'})
                $('.sup-index-no').css({'display':'none'});
            }
            // $.ajax({
            //     type: "GET",
